...
def q9
  ...
  # 以下に回答を記載
  puts names.map.with_index(1) { |name, i| "会員No.#{i} #{name}さん" }
end
...
def q11
  ...
  # 以下に回答を記載
  sports.flatten.uniq.map!.with_index(1) { |sport, i| puts "No#{i} #{sport}" }
...
def q16
  ...
  users.each do |params|
    puts "私の名前は#{params[:name]}です。年齢は#{params[:age]}歳です。"
  end
end
